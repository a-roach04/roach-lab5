...
teacher <- read_csv("data-raw/teacher_evals.csv")
```
...
> This dataset provides information of a teacher's preformance rating based on student answers but also looks at factors such as the teacher's gender and the course. Each rating, such as the teacher score and the student's grade, was recorded on a 1-5 scale. Other information, like gender and the weekday of the course, was recorded as a character. Overall, there are 8015 observations recorded. 
...
> The unit of observation is the question number for each teacher. For example, the first row is responses for question one about teacher 54655. The second row is responses for question 3 about teacher 54655.
...
teacher_evals_clean <- teacher |>
  ...
  mutate(across(teacher_id | question_no, as.character)) |> 
...
write_csv(x = teacher_evals_clean, "data-clean/teacher_evals_clean.csv")
```
...
# code chunk for Q5
teacher_clean <- read_csv("data-clean/teacher_evals_clean.csv")
...
> There are 939 unique courses and 297 unique teachers.
...
# code chunk for Q6
teacher_clean |>
  filter(if_any(.cols = -teacher_id, 
                .fns = is.na))
```
           
> Teacher 56347, course PAB3SE004PA, is missing values for percent_failed_cur.                
...
# code chunk for Q7
teacher_clean |> 
  select(academic_degree, seniority, sex, teacher_id) |> 
  group_by(sex, academic_degree) |>
  summarise(mean_seniority = mean(seniority),
            n = n())
```
             
> The majority of teachers, when split by sex, have a dr degree. For males, those with a dr degree tend to have slightly more experience than teachers with a masters. For females, teachers with a masters degree have slightly less experience than those with a dr degree. For both males and females, those with no degrees have the least amount of experience.
...
# code chunk for Q8
teacher_clean |> 
  select(question_no, teacher_id, course_id) |> 
  group_by(teacher_id, course_id) |>
  summarise(n = n_distinct(question_no)) |>
  arrange(desc(n))
```
...
teacher_clean |> 
  ...
  group_by(teacher_id) |> 
  summarise(avg_rank = mean(SET_score_avg)) |> 
  arrange(avg_rank)
```
                     
> Teachers 3682, 36250, 37013, 37023, 50714, 50740, 50759, 50790, 54309, 54665, 57254, 59338, 61502, 63917, 66073, 66244, 66275, 66290, 67678, 69446 had the highest rating of 5.0000 and teacher 54201 had the lowest rating of 1.0000                    
...
# code chunk for Q10
teacher_clean |> 
  select(teacher_id, percent_failed_cur, seniority) |>
  distinct(teacher_id, percent_failed_cur, seniority) |>
  filter(seniority <= 1) |>
  group_by(teacher_id) |> 
  arrange(percent_failed_cur)
```
            
> Teachers 98650, 106126, 84689, 86222, 102379, 98651, 84688, 103092, 80974 had 0% of students fail. Teacher 104362 had 75.54% of students fail.	
...
# code chunk for Q11
teacher_clean |> 
  select(teacher_id, academic_degree, resp_share, sex) |> 
  distinct(teacher_id, academic_degree, sex, resp_share) |>
  filter(sex == "female", academic_degree != "ma" & academic_degree != "no_dgr") |>
  group_by(teacher_id) |> 
  arrange(resp_share)
```
                  
> Teacher 3485 had the lowest proportion of responders with 0.01123596. Teacher 101508 had the highest proportion of responders with 0.52173913.
